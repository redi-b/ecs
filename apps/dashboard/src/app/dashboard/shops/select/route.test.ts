...
test("shop selection validates membership before remembering and redirecting", async () => {
  ...
  const response = await POST(
    new Request("http://app.lvh.me/dashboard/shops/select", {
      body: form,
      headers: {
  ...
  assert.equal(response.headers.get("location"), "http://bole-style.lvh.me/dashboard");
...
test("shop selection does not remember a stale or unauthorized membership", async () => {
  ...
  const response = await POST(
    new Request("http://app.lvh.me/dashboard/shops/select", { body: form, method: "POST" }),
  );

  assert.equal(response.status, 303);
  assert.equal(
    response.headers.get("location"),
    "http://app.lvh.me/dashboard/shops?error=shop_not_found",
  );

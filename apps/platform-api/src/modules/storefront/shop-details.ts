...
export function createBrandedShopData(data: unknown, name: string, details: unknown): unknown {
  ...
  const parsed = shopDetailsSchema.safeParse(details);
  const contacts = parsed.success
    ? footer
    : { ...footer, phone: undefined, email: undefined, address: undefined, socialLinks: [] };
  return {
    ...source,
    header: { ...record(source.header), logoAssetId: undefined, useShopName: false },
    footer: { ...contacts, blurb: parsed.data?.description || `Explore ${name}.` },
    ...(record(source.listing)
      ? { listing: { ...record(source.listing), body: "Browse our available products." } }
      : {}),
    ...(home && hero
      ? {
          home: {
              ...
              ...(typeof hero.body === "string"
                ? {
                    body:
                      ...
                      "Explore our products and find something for you.",
                  }
                : {}),
            ...
            ...(record(home.quality)
              ? {
                  quality: {
                    ...
                    body:
                      parsed.data?.description ||
                      "Explore our products and contact our shop for help.",
                    accordion1Title: "Product information",
                    accordion1Body:
                      "Check each product for its specifications and available options.",
                    accordion2Title: "Returns and warranty",
                    accordion2Body:
                      "Contact the shop for the terms that apply to your product before ordering.",
                  },
                }
              : {}),
            ...(record(home.featuredItem)
              ? {
                  featuredItem: {
                    ...record(home.featuredItem),
                    title: "Featured products",
                    body: "A closer look at products from our shop.",
                  },
                }
              : {}),
            ...(record(home.categories)
              ? { categories: { ...record(home.categories), title: "Browse our collections" } }
              : {}),
            ...(record(home.bestSellers)
              ? { bestSellers: { ...record(home.bestSellers), title: "Our products" } }
              : {}),
            ...(record(home.contact)
              ? {
                  contact: {
                    ...record(home.contact),
                    body: `Contact ${name} for product questions and help with your order.`,
                  },
                }
              : {}),
            ...(record(home.cta)
              ? { cta: { ...record(home.cta), title: "Find your next purchase" } }
              : {}),
          },
        }
      : {}),

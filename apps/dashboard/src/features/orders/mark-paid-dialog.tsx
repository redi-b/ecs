...
export type SettlementMethodUi = "cash" | "telebirr" | "cbe_birr" | "bank_transfer" | "other";
...
export function MarkPaidDialog({
  ...
  const accountOptions = useMemo(
    () =>
      accounts.map((account) => ({
        value: account.id,
        label: account.accountLast4 ? `${account.label} ···${account.accountLast4}` : account.label,
  ...
  return (
    <>
      ...
      >
        <DialogContent className="flex max-h-[min(92dvh,40rem)] flex-col gap-0 overflow-visible p-0 sm:max-w-md">

...
function normalizeItems(value: unknown): StoreCustomerWishlistEntry[] | null {
  ...
  for (const value of raw) {
    ...
    if (typeof item.path !== "string" || !item.path.startsWith("/products/") || item.path.length > 500) return null;
    items.push({
      path: item.path,
      title: typeof item.title === "string" && item.title.trim() ? item.title.slice(0, 200) : "Product",
      thumbnail: typeof item.thumbnail === "string" ? item.thumbnail.slice(0, 2_000) : null,
      priceAmount: typeof item.priceAmount === "number" && Number.isFinite(item.priceAmount) && item.priceAmount >= 0 ? item.priceAmount : null,

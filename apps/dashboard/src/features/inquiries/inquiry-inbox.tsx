...
} from "@/lib/platform-api/inquiries/client";
...
export function InquiryInbox({
  ...
  async function updateStatus(nextStatus: StorefrontInquiryStatus) {
    ...
    startTransition(async () => {
      ...
      const response = await fetch(`${dashboardRoutes.inquiryAction(selected.id)}${suffix}`, {
        method: "PATCH",
          ...
          <div className="divide-y divide-border/70">
            {inquiries.map((inquiry) => (
              <button
                ...
                type="button"
                onClick={() => {
                  setSelected(inquiry.status === "new" ? { ...inquiry, status: "read" } : inquiry);
                  if (inquiry.status === "new")
                    setTimeout(() => void updateStatusFor(inquiry, "read", tenantId, router), 0);
                }}
                className="group grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-3 px-4 py-4 text-left transition-colors hover:bg-muted/35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset sm:px-5"
              >
                <span
                  className={cn(
                    "mt-1 size-2 rounded-full",
                    inquiry.status === "new" ? "bg-primary" : "bg-muted-foreground/25",
                  )}
                />
            ...
            <div className="flex items-center gap-2">
              ...
              {selected && (
                <Badge
                  ...
                  variant={selected.status === "new" ? "default" : "secondary"}
                >
              ...
              {Object.entries(selected.details)
                ...
                .map(([key, value]) => (
                  <Detail
                    ...
                    label={formatKey(key)}
                    value={value}
                    href={key === "productUrl" ? value : undefined}
                  />
                ))}
            </SheetBody>
          )}
...
async function updateStatusFor(
  ...
  const suffix = tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : "";
  const response = await fetch(`${dashboardRoutes.inquiryAction(inquiry.id)}${suffix}`, {
    method: "PATCH",
    headers: { "content-type": "application/json", accept: "application/json" },

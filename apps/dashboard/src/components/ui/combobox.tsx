...
function ComboboxContent({
  ...
  return (
    <ComboboxPrimitive.Portal className="contents" container={modalContainer ?? undefined}>
      ...
      >
        <ComboboxPrimitive.Popup
          ...
          data-chips={!!anchor}
          className={cn(
            "group/combobox-content relative max-h-(--available-height) w-(--anchor-width) max-w-(--available-width) min-w-[calc(var(--anchor-width)+--spacing(7))] origin-(--transform-origin) overflow-hidden rounded-xl bg-popover text-popover-foreground shadow-lg ring-1 ring-foreground/10 duration-100",
            "data-[chips=true]:min-w-(--anchor-width)",
            "data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
            "*:data-[slot=input-group]:m-1.5 *:data-[slot=input-group]:mb-0 *:data-[slot=input-group]:h-9 *:data-[slot=input-group]:border-input/40 *:data-[slot=input-group]:bg-muted/40 *:data-[slot=input-group]:shadow-none",
...
function ComboboxItem({ className, children, ...props }: ComboboxPrimitive.Item.Props) {
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-pointer items-center gap-2 rounded-lg py-2 pr-9 pl-2.5 text-sm outline-hidden select-none",
...
function ComboboxEmpty({ className, ...props }: ComboboxPrimitive.Empty.Props) {
  return (
    <ComboboxPrimitive.Empty
      ...
      className={cn(
        "hidden w-full justify-center px-3 py-6 text-center text-sm text-muted-foreground group-data-empty/combobox-content:flex",
        className,
      )}
      {...props}
    />
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "flex h-6 w-fit max-w-full items-center justify-center gap-1 rounded-full bg-secondary px-2 text-xs font-medium whitespace-nowrap text-secondary-foreground",
    ...
    >
      ...
      {showRemove && (
        <ComboboxPrimitive.ChipRemove
          ...
          className="size-5 shrink-0 rounded-full opacity-60 hover:bg-background/60 hover:opacity-100"

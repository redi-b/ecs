...
type Delivery = DeliverySettings["delivery"];
...
function SettingsLinkRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 text-sm">
      <span className="text-muted-foreground">{label}</span>
      <a
...
function StorefrontTemplateOption({
  currentTemplateKey,
  template,
  tenantId,
}: {
...
function StorefrontTemplatePreview({ template }: { template: StorefrontTemplateCatalogItem }) {
  return (
...
function getTemplateTags(template: StorefrontTemplateCatalogItem) {

...
type StorefrontVisualEditorProps = {
...
const useStorefrontPuck = createUsePuck();
const HISTORY_LIMIT = 60;
const HISTORY_COMMIT_DELAY_MS = 700;
const FONT_OPTIONS = ["Inter", "Geist", "Manrope", "DM Sans", "Nunito Sans", "Source Sans 3"];
const POPOVER_MOTION_CLASSNAME =
...
function PuckDataOverride({ data }: { data: Data | null }) {
...
function StorefrontEditorActions({
  canRedo,
  canUndo,
  editorMeta,
  isFullscreen,
...
function StorefrontEditorShell({
  canRedo,
  canUndo,
  editorMeta,
  isFullscreen,
...
function StorefrontSettingControl({
  data,
  dispatch,
  field,
  value,
...
function UnsupportedTemplatePreview({ templateKey }: { templateKey: string }) {
  return (
...
function ClassicV1StorefrontPreview(props: StorefrontPageProps & { storefrontName?: string }) {
  const theme = {
...
function preventPreviewLink(event: React.MouseEvent<HTMLAnchorElement>) {

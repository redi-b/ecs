...
test("POST /sign-out signs out through Platform auth and forwards clearing cookies", async () => {
  ...
  const response = await POST(
    new Request("http://dashboard.test/sign-out", {
      headers: {
  ...
  assert.equal(response.headers.get("location"), "http://app.lvh.me/sign-in");
...
test("POST /sign-out preserves a safe local destination", async () => {
  process.env.PLATFORM_API_BASE_URL = "http://platform.test";
  process.env.DASHBOARD_PUBLIC_BASE_URL = "http://app.lvh.me";
  ...
  const response = await POST(
    new Request("http://dashboard.test/sign-out", {
      body: new URLSearchParams({
        next: "/sign-in?next=%2Faccept-invitation%3FinvitationId%3Dinvite_123",
      }),
      method: "POST",
    }),
  );
  ...
  assert.equal(
    ...
    "http://app.lvh.me/sign-in?next=%2Faccept-invitation%3FinvitationId%3Dinvite_123",
  );
});

test("POST /sign-out rejects an external destination", async () => {
  process.env.PLATFORM_API_BASE_URL = "http://platform.test";
  process.env.DASHBOARD_PUBLIC_BASE_URL = "http://app.lvh.me";
  globalThis.fetch = async () => Response.json({ success: true });
